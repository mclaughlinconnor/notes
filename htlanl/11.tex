...
\begin{itemize}
	...
	\item \mintinline{java}{items.set(index, element)} can be used to set elements too \mintinline{java}{items.add(index, element)} adds elements. Also only works if that position has had data in it before.
	...
	\item The search functions - \mintinline{java}{items.contains(element)} in addition to \mintinline{java}{items.indexOf(element)} and \mintinline{java}{items.lastIndexOf(element)} - can be used to search through an \mintinline{java}{ArrayList} to find \mintinline{java}{element}.
\end{itemize}
