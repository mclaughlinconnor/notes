...
\tikz[
    ...
    \node[attribute] (atrone) {Attribute One};
    \node[attribute] (atrtwo) {Attribute Two};
    \node[attribute] (atrthree) {Attribute Three};
    \node[attribute] (atrfour) {Attribute Four};

    \draw (entity) edge (atrone);
    \draw (entity) edge (atrtwo);
    \draw (atrtwo) edge (atrthree);
    \draw (atrtwo) edge (atrfour);
}
\end{document}
